            ...
            match &self.attributes.state {
                ...
                AgentState::UnitTesting => {
                    ...
                    */
                    ...
                    // Execute running server to spawn new process and run cargo
                    ...
                    // Check status code if there an endpoint
                    for endpoint in check_endpoints {
                        ...
                        // Test url to make request to endpoint on 8080
                        ...
                        match check_status_code(&client, &url).await {
                            Ok(status_code) => {
                                if status_code != 200 {
                                    ...
                                    // if error, then issue print command
                                    PrintCommand::Issue.print_agent_message(
                                        self.attributes.position.as_str(),
                                        err_msg.as_str(),
                                    );
                            ...
                            // print error
                            Err(e) => {
                                // kill process on port 8080 $(lsof -t -i:8080)
                                run_backend_server
                                    .kill()
                    ...
                    // then kill the server upon completion
                    run_backend_server
                        .kill()
                        .expect("Failed to kill backend web server on completion");
                    // tell agent that AI is done
...
mod tests {
    ...
    async fn tests_backend_developer() {
        ...
        let factsheet_str: &str = r#"
      {
        ...
        // test only within agent backend
